                    ...
                    <div class="card-body">
                        <form
                            ...
                            <div class="gy-4 mb-4">
                                ...
                                <div class="row col-xl-9 col-sm-12 mb-3">
                                    <label for="body"
                                        class="form-label col-xl-2 col-lg-2 col-md-2 col-sm-2">Body</label>
                                    <div class="col-xl-8 col-lg-8 col-md-8 col-sm-12">
    ...
    <script>
        $(document).ready(function() {
            function toggleFields() {
                ...
                const isEditing = @json(isset($announcement));

                if (!isEditing) {
                    $('#status').closest('.row').hide(); // Hide status when creating
                    $('#published_at_wrapper').hide(); // Hide publish date when creating
                } else {
                    $('#status').closest('.row').show(); // Show status when editing

                    if (status === 'Inactive') {
                        $('#published_at_wrapper').closest('.row').hide();
                    }
                    if (status === 'Active') {
                        $('#published_at_wrapper').show(); // Show publish date when status is active
                        if (!$('#published_at').val()) {
                            // Auto-fill current date if the field is empty
                            const currentDate = new Date().toISOString().slice(0, 16);
                            $('#published_at').val(currentDate);
                        }
                    } else {
                        $('#published_at_wrapper').hide(); // Hide publish date when status is inactive
                    }
                }
            }

            // Initially toggle fields based on current values
            toggleFields();
